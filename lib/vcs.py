...
def _do(cmd, caption, path, out=None, ask=None, **kwargs):
    ...
    if ask and not sublime.ok_cancel_dialog(replacekeys(ask, {'path': base_name}), caption):
      return
    if out == 'gui':
...
def call(cmd, active_flags, view):
    ...
    for item in scslist:
      ...
      if cmdi is not None and \
         suricate.flags.special_check(active_flags, suricate.flags.from_string(item['flags'])) and \
         all(suricate.util.which(exe) is not None for exe in item['exes']):
...
def get_commands(active_flags, path):
    ...
    commandlist = []
    for item in scslist:
      if suricate.flags.special_check(active_flags, suricate.flags.from_string(item['flags'])) and \
         all(suricate.util.which(exe) is not None for exe in item['exes']):
